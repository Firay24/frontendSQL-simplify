...
import {
  getFlock, getFunctional, getFunctionals, getClasses, getSuluks, getNotesFlock, updateFlock, updateFunctional, updateClass, updateSuluk, updateNoteFlockBio,
} from 'utils/apiData';
import {
...
function EditPage() {
  ...
  useEffect(() => {
    if (status.updateFlock || status.updateClass || status.updateSuluk || status.updateFunctional) {
      setTimeout(() => {
        navigate(`/fungsional/detailData/${idFlock}`);
      }, 2000);
    }
  }, [status]);
  ...
  useEffect(() => {
    if (idFunc) {
      const fetchData = async (idParams) => {
        try {
          const result = await getFunctional(idParams);
  ...
  const detailFunctional = functional && functional.data && functional.data.functional;
  ...
  useEffect(() => {
    if (detailFlock && dataFunctionals) {
      const detailsFunctional = detailFlock && dataFunctionals && dataFunctionals.find((functionalData) => functionalData.nik === detailFlock.nik && functionalData.fathersName === detailFlock.fathersName);
      if (detailsFunctional) {
        setFunctional(detailsFunctional);
      }
    }
    ...
    <div className="mt-4 mr-10 mb-6">
      ...
      <div>
        <EditDataSection
          ...
          suluk={suluk && suluk}
          functional={idFunc ? detailFunctional && detailFunctional : functional && functional}
          notes={note && note}
